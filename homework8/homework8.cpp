...
protected:
	void SetUp() override 
	{
		_st_group = new StudentsGroup (std::vector<Student> {
	...
	void TearDown() override { delete _st_group; }
	StudentsGroup* _st_group;
};
class SGTest2 : public testing::Test
{
protected:
	void SetUp() override
	{
		_st_group = new StudentsGroup();
	}
	void TearDown() override { delete _st_group; }
	StudentsGroup* _st_group;
};
TEST_F(SGTest, average_score)
{
	ASSERT_EQ(7, _st_group->GetAverageScore(FullName("Trofimova", "Elena", "Sergeevna")));
	ASSERT_EQ(8, _st_group->GetAverageScore(FullName("Rodionov", "Aleksey", "Ivanovich")));
	ASSERT_EQ(6, _st_group->GetAverageScore(FullName("Semina", "Ekaterina", "Vladimirovna")));
}
TEST_F(SGTest, all_info)
{
	ASSERT_STREQ(_st_group->GetAllInfo().c_str(), "Semina Ekaterina Vladimirovna 7 2 8 6 9 \nTrofimova Elena Sergeevna 8 9 5 6 10 \nRodionov Aleksey Ivanovich 9 8 7 9 8 \n");
}
TEST_F(SGTest, serialization)
{
	_st_group->Save();
	std::ifstream out("students.bin", std::ios_base::binary);
	ASSERT_TRUE(out.is_open()&&out.badbit);
	out.close();
}
